...
module.exports = {
    ...
    run: async (client, message, args, prefix, command) => {
        ...
        client.Transcript.findOne({ Author: message.author.id }, async(err, data) => {
            if (err) {
            ...
            if (data) {
                ...
                message.guild.channels.cache.get(data.Channel).updateOverwrite(data.Author, { VIEW_CHANNEL: true })
            } else {
